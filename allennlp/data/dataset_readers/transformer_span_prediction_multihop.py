...
@DatasetReader.register("transformer_span_prediction_multihop")
class TransformerSpanPredictionMultihopReader(DatasetReader):
    """
    ...
    def _example_to_instance(self, example, debug):
        ...
        if debug > 0:
            ...
            logger.info(f"b_mask = {features.b_masks}")
            logger.info(f"s_mask = {features.s_masks}")
            logger.info(f"q_mask = {features.q_masks}")
    ...
    def _transformer_features_from_example(self, example, debug):
        ...
        orig_to_tok_index = []
        all_doc_tokens = []
        situation_start_tok_index = -1
        for (i, token) in enumerate(example.doc_tokens):
            if token == self._add_prefix.get("s", "").strip():
                situation_start_tok_index = len(all_doc_tokens)
        ...
        assert situation_start_tok_index != -1
        q_tok_to_orig_index = []
        q_orig_to_tok_index = []
        all_query_tokens = []
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if doc_span.start >= situation_start_tok_index:
            ...
            for i in range(doc_span.length):
                ...
                if split_token_index < situation_start_tok_index:
                    b_masks += [1]
                    s_masks += [0]
                else:
                    b_masks += [0]
