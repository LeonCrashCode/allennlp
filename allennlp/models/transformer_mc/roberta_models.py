...
@Model.register("roberta_span_reasoning_multihop")
class RobertaSpanReasoningMultihopModel(Model):
    """
    ...
    def forward(self,
        ...
        self._accuracy(log_probs, best)

        output_dict = {}
        output_dict["loss"] = self.loss(log_probs, best)
...
@Model.register("roberta_span_reasoning_multihop2")
class RobertaSpanReasoningMultihop2Model(Model):
    """
    ...
    def __init__(self,
        ...
        if self.ablation in [4,3,2]:
            self.linear_layer = Linear(transformer_config.hidden_size*(6-self.ablation+1), transformer_config.hidden_size)
        elif self.ablation == 1:
            self.linear_layer = Linear(transformer_config.hidden_size*5, transformer_config.hidden_size)
        else:
            self.linear_layer = Linear(transformer_config.hidden_size*6, transformer_config.hidden_size)

        self.scorer = Linear(transformer_config.hidden_size, 1)
        self.span_extractor = EndpointSpanExtractor(input_dim=transformer_config.hidden_size)
        self.dropout = torch.nn.Dropout(p=dropout)
    ...
    def forward(self,
        ...
        if self.ablation == 4: #c q
            if self.sumpool:
                reps = cands_reps + Q_reps.unsqueeze(1).expand(-1, cands_num, -1)
            else:
                reps = torch.cat((cands_reps, Q_reps.unsqueeze(1).expand(-1, cands_num, -1)), dim=-1)
        elif self.ablation == 3: #c q b1
            if self.sumpool:
                reps = cands_reps + B_reps1 + Q_reps.unsqueeze(1).expand(-1, cands_num, -1)
            else:
                reps = torch.cat((cands_reps, B_reps1, Q_reps.unsqueeze(1).expand(-1, cands_num, -1)), dim=-1)
        elif self.ablation == 2: #c q b1 b2
            if self.sumpool:
                reps = cands_reps + B_reps1 + B_reps2 + Q_reps.unsqueeze(1).expand(-1, cands_num, -1)
            else:
                reps = torch.cat((cands_reps, B_reps1, B_reps2, Q_reps.unsqueeze(1).expand(-1, cands_num, -1)), dim=-1)
        elif self.ablation == 1:
            if self.sumpool:
                reps = cands_reps + B_reps1 + S_reps + Q_reps.unsqueeze(1).expand(-1, cands_num, -1)
            else:
                reps = torch.cat((cands_reps, B_reps1, S_reps, Q_reps.unsqueeze(1).expand(-1, cands_num, -1)), dim=-1)
        else: #q b1 b2 s
            if self.sumpool:
                reps = cands_reps + B_reps1 + B_reps2 + S_reps + Q_reps.unsqueeze(1).expand(-1, cands_num, -1)
            else:
                reps = torch.cat((cands_reps, B_reps1, B_reps2, S_reps, Q_reps.unsqueeze(1).expand(-1, cands_num, -1)), dim=-1)

        reps = self.linear_layer(reps)
        reps = self.tanh(reps)
...
@Model.register("roberta_span_reasoning_multihop3")
class RobertaSpanReasoningMultihop3Model(Model):
    """
    ...
    def forward(self,
        ...
        #exit(-1)
        B_weights2 = self.B_mlp(B_reps2).squeeze(-1)
        B_weights2 = B_weights2.masked_fill((s_masks <= 0), -1e18)
        #B_weights2 = B_weights2.masked_fill((b_masks <= 0), -1e18)
        B_attn2 = torch.softmax(B_weights2, dim=-1)
        output_dict["b_attn2_self"] = B_attn2.masked_fill((s_masks <= 0), 0)
        #output_dict["b_attn2_self"] = B_attn2.masked_fill((b_masks <= 0), 0)
        B_reps2 = torch.matmul(B_attn2.unsqueeze(1), B_reps2)
        #B_reps2 = torch.matmul(B_attn2.unsqueeze(1), sequence_output)
        # B x 1 x Dim
        B_reps2 = self.dropout(B_reps2)
...
@Model.register("roberta_span_reasoning_multihop4")
class RobertaSpanReasoningMultihop4Model(Model):
    """
...
class RobertaSpanReasoningMultihop44Model(Model):
    ...
    def forward(self,
        ...
        # STEP2, get B_attn1
        # B x 1 x Dim, B x 1 x L, B x L x Dim
        B_reps1, B_attn1, _ = self.B1_multi_head_attention(key=sequence_output, value=sequence_output, query=Q_reps.unsqueeze(1), mask=b_masks.unsqueeze(1))
        B_reps1 = self.dropout(B_reps1)
        output_dict["b_attn1"] = B_attn1

        S_reps, S_attn, _ = self.S_multi_head_attention(key=sequence_output, value=sequence_output, query=Q_reps.unsqueeze(1), mask=s_masks.unsqueeze(1))
        S_reps = self.dropout(S_reps)
...
@Model.register("roberta_span_reasoning_multihop45")
class RobertaSpanReasoningMultihop45Model(Model):
    """
    C + Q + Q->B + Q->B->S
    """
    def __init__(self,
    ...
    def forward(self,
        ...
        B_reps1 = self.CB_mlp(B_reps1)

        #B x cands_num x Dim
        B_reps1 = self.dropout(B_reps1)

        #B x cands_num x Dim, B x cands_num x L
        S_reps, S_attn, _ = self.S_multi_head_attention(key=sequence_output, value=sequence_output, query=B_reps1, mask=s_masks.unsqueeze(1).expand(-1, cands_num, -1))
        output_dict["s_attn"] = S_attn
        S_reps = self.dropout(S_reps)
...
@Model.register("roberta_span_reasoning_multihop46")
class RobertaSpanReasoningMultihop46Model(Model):
    """
    C + Q + Q->B + Q->B | B ->S
    """
    def __init__(self,
...
@Model.register("roberta_span_reasoning_multihop47")
class RobertaSpanReasoningMultihop47Model(Model):
    """
    C + Q + C->B
    """
    def __init__(self,
...
@Model.register("roberta_span_reasoning_multihop5")
class RobertaSpanReasoningMultihop5Model(Model):
    """
    ...
    def forward(self,
        ...
        S_weights = self.B_mlp(sequence_output).squeeze(-1)
        S_weights = S_weights.masked_fill((s_masks <= 0), -1e18)
        S_attn = torch.softmax(S_weights, dim=-1)
        
        S_reps = torch.matmul(S_attn.unsqueeze(1), sequence_output).squeeze(1)
        S_reps = self.dropout(S_reps)

        B_reps2, B_attn2, expanded_B_reps2 = self.B1_multi_head_attention(key=sequence_output, value=sequence_output, query=S_reps.unsqueeze(1), mask=b_masks.unsqueeze(1))
        B_reps2 = self.dropout(B_reps2)
        output_dict["b_attn2"] = B_attn2
        # STEP4, get S_attn
